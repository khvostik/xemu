...
void PopupMenu::Draw(PopupMenuItemDelegate &nav)
{
    ...
    if (ImGui::Begin("###PopupMenu", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings)) {
        if (DrawItems(nav)) nav.PopMenu();
        if (!ImGui::IsWindowFocused()) nav.LostFocus();
    }
    ImGui::End();
